...
查看 [Hint](https://learnku.com/docs/gorm/v2/hints) 获取详情
...
查看 [自定义数据类型](https://learnku.com/docs/gorm/v2/data_types#gorm_valuer_interface) 获取详情
...
查看 [Database Resolver](https://learnku.com/docs/gorm/v2/dbresolver) 获取详情

#### Prometheus

GORM 提供了 `Prometheus` 插件来收集 `DBStats` 和用户自定义指标

查看 [Prometheus](https://learnku.com/docs/gorm/v2/prometheus) 获取详情

#### 命名策略

GORM 允许用户通过覆盖默认的`命名策略`更改默认的命名约定，命名策略被用于构建： `TableName`、`ColumnName`、`JoinTableName`、`RelationshipFKName`、`CheckerName`、`IndexName`。查看 [GORM 配置](https://learnku.com/docs/gorm/v2/gorm_config) 获取详情
...
下面以 JSON 为例（支持 SQLite、MySQL、Postgres。参考自：[https://github.com/go-gorm/datamypes/blob/master/json.go）](https://github.com/go-gorm/datamypes/blob/master/json.go）)
...
GORM 可以通过 [`Select`](https://learnku.com/docs/gorm/v2/query) 选择指定的字段，而在 V2 中，通过一个较小的 struct，可以使用 GORM 提供的 smart select 模式
...
* GORM V2 使用 `camelCase` 风格的 tag 名。`snake_case` 风格的 tag 已经失效，例如： `auto_increment`、`unique_index`、`polymorphic_value`、`embeded_prefix`，查看 [Model Tag](https://learnku.com/docs/gorm/v2/models#tags) 获取详情
* 用于指定外键的 tag 已变更为 `foreignKey`，`references`，查看 [Association Tag](https://learnku.com/docs/gorm/v2/associations#tags) 获取详情

#### Table Name

`TableName` _不再_ 允许动态表名， 因为 `TableName` 的返回值会被缓存下来
...
查看 [方法链](https://learnku.com/docs/gorm/v2/method_chaining) 获取详情

#### 默认值

创建记录后，GORM V2 不会自动加载由数据库生成的默认值，查看 [默认值](https://learnku.com/docs/gorm/v2/create#default_values) 获取详情
...
在 V2 中，Before/After Create/Update/Save/Find/Delete 必须定义为 `func(tx *gorm.DB) error` 类型的方法，这是类似于插件 callback 的统一接口。如果定义为其它类型，它不会生效，并且会打印一个警告日志，查看 [Hook](https://learnku.com/docs/gorm/v2/hooks) 获取详情
...
插件 callback 也需要被定义为 `func(tx *gorm.DB) error` 类型的方法，查看 [Write Plugins](https://learnku.com/docs/gorm/v2/write_plugins) 获取详情
...
查看 [事务](https://learnku.com/docs/gorm/v2/transactions) 获取详情
...
查看 [Migration](https://learnku.com/docs/gorm/v2/migration) 获取详情
