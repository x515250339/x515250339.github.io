...
3. vim .bash\_profile  \# 家目录下编辑.bash\_profile文件,加入以下3句
...
第二句话是 需要填写 本机python3 的安装位置 具体可以 which python3![](/assets/import1.png)

第三句话是 需要填写 之前安装的虚拟机的 virtualenvwrapper.sh 的位置 具体可以 which virtualenvwrapper.sh source是更新这个文件使其生效

![](/assets/import2.png)

1. 保存运行这个命令 source ~/.bash\_profile
2. 目前为止已经安装完毕,加下来新建工作目录\(npy 为新建的虚拟环境\)
3. 新建虚拟环境:mkvirtualenv -p python3 npy  \#新建成功后,当前路径前面就会有npy
